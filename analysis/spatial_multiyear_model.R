...
model_code <- nimbleCode({
  ...
  for (i in 1:K) { # loop over sites
    for (t in 1:M) { # loop over years
      ...
      # likelihood: process
      N[t, i] ~ dpois(lambda[i]) # for each site
...
out <- clusterEvalQ(cl, {
  ...
  # build the MCMC
  ModSpec <- configureMCMC(Rmodel, onlySlice = TRUE,
                           monitors = c("alpha", "N", "beta", "sigma_s", 
                                        "s_s"#,
                                        #"fit1.data","fit1.pred","fit2.data",
                                        #"fit2.pred"
                                        )
                           )
...
# save samples
saveRDS(out_sub, "samples/samples_spatial.rds")
