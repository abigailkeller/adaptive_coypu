...
model_code <- nimbleCode({
  ...
  for (i in 1:K) { # loop over sites
    for (t in 1:M) { # loop over years
      ...
      y[t, i, 1:J] ~ dmulti(pic[i, 1:J], n[t, i]) # multinomial for each site
      n[t, i] ~ dbin(pcap[i], N[t, i]) # for each site
      # likelihood: process
  ...
  s_t[1:M] ~ dcar_normal(adj_t[1:L_t], weights_t[1:L_t], num_t[1:M], 
                         tau_t, zero_mean = 0)
...
# save samples
saveRDS(samp, "samples/samples_spatiotemporal.rds")
