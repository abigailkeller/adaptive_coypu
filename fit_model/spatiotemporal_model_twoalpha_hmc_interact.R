...
# get temporal neighborhood structure
nbInfo_time <- list()
nbInfo_time$adj <- c(2, 
                     1, 3, 
                     2, 4, 
                     ...
                     7, 9,
                     8)
nbInfo_time$num <- c(1, rep(2, dim(dat_st)[1] - 2), 1)
nbInfo_time$weights <- rep(1, length(nbInfo_time$adj))
...
constants <- list(K = K, J = J, M = M,
                  ...
                  num_s = nbInfo$num,
                  L_t = length(nbInfo_time$adj), 
                  adj_t = nbInfo_time$adj, weights_t = nbInfo_time$weights, 
                  num_t = nbInfo_time$num) 
data <- list(
  y = dat_st,
  n = n,
...
model_code <- nimbleCode({
  ...
  for (t in 1:M) { # loop over years
    ...
    for (i in 1:K) { # loop over sites
      ...
      log(lambda[t, i]) <- beta[1] + beta[2] * temp[t, i] + s_s[i] + s_t[t, i]
  ...
  # spatio-temporal latent process (type II)
  for (i in 1:K){
    s_t[1:M, i] ~ dcar_normal(adj_t[1:L_t], weights_t[1:L_t], num_t[1:M], 
                              tau_t, zero_mean = 1) 
  }
  sigma_t ~ dunif(0, 100)   # prior for variance components based on Gelman (2006)
  tau_t <- 1 / sigma_t ^ 2
...
# save samples
saveRDS(out_sub, "samples/samples_spatiotemporal_rw_twoalpha_hmc_interact.rds")
