...
model_code <- nimbleCode({
  ...
  for (i in 1:K) {
    ...
    for (t in 2:M) {
      ...
      log(lambda[t, i]) <- (
        s_t[t] + 
          rho * log(remaining[t - 1, i] + 1) +
          gamma * log(neighbors_rem[t - 1, i])
      )
...
# save samples
saveRDS(out_sub, "samples/samples_spillover_timeranef_rw_twoalpha_notemp.rds")
