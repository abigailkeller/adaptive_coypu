...
model_code <- nimbleCode({
  ...
  for (i in 1:K) {
    ...
    for (t in 2:M) {
      ...
      neighbors_rem[t - 1, i] <- get_sum(remaining[t - 1, 1:K], 
                                          neighbors[i, 1:max_neighbors], 
                                          num_s[i])
      
      log(lambda[t, i]) <- (
        beta * temp[t, i] + s_t[t] + 
          log(gamma * rho * remaining[t - 1, i] + 1 +
                (1 - gamma) * rho * neighbors_rem[t - 1, i])
      )
...
# save samples
saveRDS(out_sub, "samples/samples_spillover_timeranef_rw_twoalpha_sum.rds")
