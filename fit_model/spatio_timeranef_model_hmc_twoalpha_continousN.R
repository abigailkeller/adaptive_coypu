...
model_code <- nimbleCode({
  ...
  for (t in 1:M) { # loop over years
    ...
    for (i in 1:K) { # loop over sites
      ...
      n[t, i] ~ dbin(pcap[t, i], round(N[t, i])) # for each site
      # likelihood: process
      N[t, i] ~ dgamma(shape = lambda[t, i], rate = 1) # for each site
...
# save samples
saveRDS(out_sub, "samples/samples_spatio_timeranef_hmc_twoalpha_contN.rds")
