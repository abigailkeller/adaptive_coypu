...
neighbors <- matrix(NA, nrow = dim(dat_st)[2], ncol = max(nbInfo$num))
start <- 1
for (i in 1:dim(dat_st)[2]) {
  
  end <- start + nbInfo$num[i] - 1
  neighbors[i, 1:nbInfo$num[i]] <- nbInfo$adj[start:end]
...
model_code <- nimbleCode({
  ...
  for (i in 1:K) {
    ...
    for (t in 2:M) {
      ...
      # get total
      col[t - 1, i] <- gamma * neighbors_rem[t - 1, i]
      
      log(lambda[t, i]) <- (
        s_t[t] + 
          log((rho * remaining[t - 1, i] + col[t - 1, i]) * 
                (1 - (remaining[t - 1, i] + col[t - 1, i]) / exp(dd[i])) + 1)
      )
...
# save samples
saveRDS(out_sub, "samples/samples_timeranef_rw_twoalpha_dd_nobeta_space_stzero.rds")
